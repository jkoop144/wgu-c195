...
import static jkupidu_c195.addAppointmentController.selectedCustomer;
import static jkupidu_c195.calendarMainController.passAppointmentIndex;
...
public class editAppointmentController implements Initializable {
    ...
    private Appointment appointmentSelected = passAppointmentIndex();
    ...
    public void saveButton(ActionEvent event) throws IOException {
        ...
        try {
           ...
           if (errorMessage.length() > 0){
                ...
                error.showAndWait();
            } else if (Database.scheduleCheck(appointmentSelected.getAppointmentId(), location.getText(), dateSelected.toString(), timeList.get(timeSelected))) {
                Alert error = new Alert(Alert.AlertType.INFORMATION);
                error.setTitle("Error");
                ...
                Database.editAppointment(appointmentSelected.getAppointmentId(), typeList.get(typeSelected), contactList.get(contactSelected), location.getText(), dateSelected.toString(), timeList.get(timeSelected),customerId);
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        //Set fields
        String typeInfo = appointmentSelected.getTitle() + "-" + appointmentSelected.getDescription();
        //customerNames.getSelectionModel().select(appointmentSelected.getCustomerId()-1);
        contact.setValue(appointmentSelected.getContact());
        location.setText(appointmentSelected.getLocation());
        type.setValue(typeInfo);
        time.setValue(appointmentSelected.getTimeOnlyString());
        date.setValue(appointmentSelected.getDateInFormat());
        
        
        //This Lamba makes it easy for me to block out dates on the date picker
